...
namespace CyberCar {
 ...
 VehicleController car;Material brakeMaterial,smokeMaterial;AudioSource tires;AudioClip tireClip;
 ...
 car=owner;
 brakeMaterial=new Material(Shader.Find("Standard")){name="Responsive rear lamps"};
 brakeMaterial.EnableKeyword("_EMISSION");brakeMaterial.SetFloat("_Glossiness",.7f);
 foreach(var r in GetComponentsInChildren<Renderer>())if(r.name.StartsWith("Taillight")||r.name=="Rear light bar")r.sharedMaterial=brakeMaterial;
 ...
 foreach(var headlight in headlights)headlight.enabled=car.Session!=null&&car.Session.Night;
 BrakeLightsActive=driving&&(car.Brake||(car.Throttle<0&&car.Speed>1)||(car.IsPlayer&&car.Session.ReplayBraking));
 brakeMaterial.color=BrakeLightsActive?new Color(1,.025f,.012f):new Color(.25f,.018f,.012f);
 brakeMaterial.SetColor("_EmissionColor",new Color(1,.015f,.006f)*(BrakeLightsActive?2.2f:.16f));
 ...
 void OnDestroy(){if(brakeMaterial)Destroy(brakeMaterial);if(smokeMaterial)Destroy(smokeMaterial);if(tireClip)Destroy(tireClip);}
 }
}
