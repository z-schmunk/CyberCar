...
namespace CyberCar {
 ...
 readonly GameSession session;readonly System.Random random;int correct;bool authorized;
 public int Attack{get;private set;}=-1;public int Stage{get;private set;}public string Evidence{get;private set;}="";
 public string[] Choices{get;private set;}=new string[3];public bool Open=>Attack>=0;
 ...
 public void Begin(int attack){if(!session.Attacks.Has((CyberAttack)attack)){session.Notify("Select an active threat to inspect its evidence.");return;}Attack=attack;Stage=0;Present();}
 public void Close(){Attack=-1;}
 public bool Choose(int choice){
 if(!Open||choice<0||choice>2)return false;
 if(!session.Attacks.Has((CyberAttack)Attack)){Close();return false;}
 if(choice!=correct){session.ElapsedPenalty(4);session.Notify("That action trusts the suspect source. Read the evidence; +4 seconds.",true);return false;}
 ...
 if(++Stage<3){Present();return true;}
 authorized=true;bool success=session.Attacks.Defend(Attack);authorized=false;if(success)Close();return success;
 }
 void Present(){
 ...
 Evidence=(Attack==12?session.Player.GetComponent<CarSensors>().Readout:evidence[Attack])+"\nSTEP 1 / CONTAIN";
 ...
 correct=random.Next(3);Choices[correct]=good;Choices[(correct+1)%3]=bad;Choices[(correct+2)%3]=other;
 }
 // Exercised by the runtime harness through the same public selection path.
 public int EvidenceAnswer {get {return correct;}}
 }
}
