...
using UnityEngine;
namespace CyberCar {
 [Serializable] public sealed class DriverProfile {public int version=3,unlocked,credits;public int[] upgrades=new int[3];public bool[] rewards=new bool[15];public bool freeplay;public bool[] badges=new bool[22];public float music=.22f;}
 ...
 foreach(string candidate in new[]{path,path+".bak"}){if(!File.Exists(candidate))continue;try{var data=JsonUtility.FromJson<DriverProfile>(File.ReadAllText(candidate));if(data==null||(data.version!=2&&data.version!=3))continue;var badges=new bool[22];if(data.badges!=null)Array.Copy(data.badges,badges,Math.Min(22,data.badges.Length));data.badges=badges;data.version=3;
